...
public:
  FilterConfig(UserMap&& users, const std::string& forward_username_header,
               ...
               Stats::Scope& scope);
  const APIKeyAuthStats& stats() const { return stats_; }
...
private:
  static APIKeyAuthStats generateStats(const std::string& prefix, Stats::Scope& scope) {
    return APIKeyAuthStats{ALL_BASIC_AUTH_STATS(POOL_COUNTER_PREFIX(scope, prefix))};
  }
...
/**
 * Per route settings for APIKeyAuth. Allows customizing users on a virtualhost\route\weighted
 * cluster level.
 */
...
class APIKeyAuthFilter : public Http::PassThroughDecoderFilter,
                        public Logger::Loggable<Logger::Id::api_key_auth> {
public:
  APIKeyAuthFilter(FilterConfigConstSharedPtr config);
